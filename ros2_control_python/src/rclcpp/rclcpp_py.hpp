...
class PyExecutor: public Executor {
 public:
  ...
  void add_node(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_ptr, bool notify) override {
    PYBIND11_OVERRIDE(
        void,
        rclcpp::Executor,
        add_node,
...
inline void init_rclcpp([[maybe_unused]] py::module &m)
{
  ...
  m.def("shutdown", [](rclcpp::Context::SharedPtr context) { return shutdown(context); });

  m.def("shutdown", [](rclcpp::Context::SharedPtr context, const std::string& reason) { return shutdown(context, reason); });
  ...
  py::enum_<LifecycleNodeInterface::CallbackReturn>(m, "CallbackReturn")
      ...
      .export_values();
  py::class_<LifecycleNodeInterface, PyLifecycleNodeInterface>(m, "LifecycleNodeInterface")
      .def(py::init<>())
  ...
  py::class_<SingleThreadedExecutor, Executor, PySingleThreadedExecutor, std::shared_ptr<SingleThreadedExecutor>>(m, "SingleThreadedExecutor")
      .def(py::init<>())
      .def("spin_some", [](SingleThreadedExecutor& self) { return self.spin_some(); })
      .def("spin_some", [](SingleThreadedExecutor& self, std::int64_t max_duration) { self.spin_some(std::chrono::nanoseconds(max_duration)); })
      .def("spin_some", [](SingleThreadedExecutor& self, double max_duration) { self.spin_some(std::chrono::nanoseconds(static_cast<std::int64_t>(max_duration))); })
