...
class PyTransmission : public Transmission {
 public:
  ...
  void configure(const std::vector<JointHandle>& joint_handles,
                 const std::vector<ActuatorHandle>& actuator_handles) override {
    PYBIND11_OVERRIDE_PURE(void, Transmission, configure, joint_handles,
                           actuator_handles);
  }

  void actuator_to_joint() override {
    PYBIND11_OVERRIDE_PURE(void, Transmission, actuator_to_joint,

    );
  }

  void joint_to_actuator() override {
    PYBIND11_OVERRIDE_PURE(void, Transmission, joint_to_actuator,

    );
  }

  std::size_t num_actuators() const override {
    PYBIND11_OVERRIDE_PURE(std::size_t, Transmission, num_actuators,

    );
  }

  std::size_t num_joints() const override {
    PYBIND11_OVERRIDE_PURE(std::size_t, Transmission, num_joints,
