...
struct type_caster<std::unique_ptr<T>> {
 public:
  ...
  static handle cast(std::unique_ptr<T> /* src */,
...
struct type_caster<std::vector<T>> {
 public:
  ...
  bool load(handle src, bool) {
    ...
    value.clear();
    try {
      for (const handle& el : l) value.emplace_back(std::move(el.cast<T&>()));
    } catch (cast_error&) {
      return false;
    }
    return true;
  ...
  static handle cast(std::vector<T>&& src, return_value_policy /* policy */,
                     handle /* parent */) {
    list l{0};
    l.inc_ref();
    for (T& el : src) l.append(std::move(el));
    src.clear();
    return l;
  }
};
...
inline void init(py::module& m) {
  ...
  py::bind_map<std::unordered_map<std::string, HardwareComponentInfo>>(
      m, "UnorderedMapStringHardwareComponentInfo");
