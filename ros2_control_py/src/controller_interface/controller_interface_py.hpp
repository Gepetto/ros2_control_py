...
class PyControllerInterface : public ControllerInterface {
 public:
  ...
  InterfaceConfiguration command_interface_configuration() const override {
    PYBIND11_OVERRIDE_PURE(InterfaceConfiguration, ControllerInterface,
  ...
  CallbackReturn on_init() override {
    PYBIND11_OVERRIDE_PURE(CallbackReturn, ControllerInterface, on_init,
  ...
  return_type update(const rclcpp::Time& time,
    ...
    PYBIND11_OVERRIDE_PURE(return_type, ControllerInterface, update, time,
                           period);
  }

  CallbackReturn on_configure(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_configure,
                      previous_state);
  }

  CallbackReturn on_cleanup(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_cleanup,
                      previous_state);
  }

  CallbackReturn on_shutdown(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_shutdown,
                      previous_state);
  }

  CallbackReturn on_activate(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_activate,
                      previous_state);
  }

  CallbackReturn on_deactivate(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_deactivate,
                      previous_state);
  }

  CallbackReturn on_error(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ControllerInterface, on_error,
                      previous_state);
  }
};

inline void init_controller_interface([[maybe_unused]] py::module& m) {
  py::class_<ControllerInterface, ControllerInterfaceBase,
             PyControllerInterface>(m, "ControllerInterface")
      .def(py::init<>())
