...
class PyChainableControllerInterface : public ChainableControllerInterface {
 public:
  ...
  on_export_reference_interfaces() override {
    PYBIND11_OVERRIDE_PURE(std::vector<hardware_interface::CommandInterface>,
  ...
  bool on_set_chained_mode(bool chained_mode) override {
    PYBIND11_OVERRIDE(bool, ChainableControllerInterface, on_set_chained_mode,
                      chained_mode);
  }

  return_type update_reference_from_subscribers(
      const rclcpp::Time& time, const rclcpp::Duration& period) override {
    PYBIND11_OVERRIDE_PURE(return_type, ChainableControllerInterface,
                           update_reference_from_subscribers, time, period
  ...
  return_type update_and_write_commands(
      const rclcpp::Time& time, const rclcpp::Duration& period) override {
    PYBIND11_OVERRIDE_PURE(return_type, ChainableControllerInterface,
                           update_and_write_commands, time, period);
  }

  InterfaceConfiguration command_interface_configuration() const override {
    PYBIND11_OVERRIDE_PURE(InterfaceConfiguration, ChainableControllerInterface,
  ...
  CallbackReturn on_init() override {
    PYBIND11_OVERRIDE_PURE(
        CallbackReturn, ChainableControllerInterface, on_init,
  ...
  CallbackReturn on_configure(
    ...
    PYBIND11_OVERRIDE(CallbackReturn, ChainableControllerInterface,
                      on_configure, previous_state);
  }

  CallbackReturn on_cleanup(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ChainableControllerInterface, on_cleanup,
                      previous_state);
  }

  CallbackReturn on_shutdown(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ChainableControllerInterface, on_shutdown,
                      previous_state);
  }

  CallbackReturn on_activate(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ChainableControllerInterface, on_activate,
                      previous_state);
  }
  ...
  CallbackReturn on_error(
      const rclcpp_lifecycle::State& previous_state) override {
    PYBIND11_OVERRIDE(CallbackReturn, ChainableControllerInterface, on_error,
                      previous_state);
  }
};
...
inline void init_chainable_controller_interface(
    [[maybe_unused]] py::module& m) {
  py::class_<ChainableControllerInterface, ControllerInterfaceBase,
             PyChainableControllerInterface>(m, "ChainableControllerInterface")
      .def(py::init<>())
      ...
      .def("is_in_chained_mode",
           &ChainableControllerInterface::is_in_chained_mode)
      .def("on_export_reference_interfaces",
           &PubChainableControllerInterface::on_export_reference_interfaces)
      .def("on_set_chained_mode",
           &PubChainableControllerInterface::on_set_chained_mode)
      .def("update_reference_from_subscribers",
           &PubChainableControllerInterface::update_reference_from_subscribers)
      .def("update_and_write_commands",
           &PubChainableControllerInterface::update_and_write_commands)
      .def("command_interface_configuration",
           &ChainableControllerInterface::command_interface_configuration)
      .def("state_interface_configuration",
           &ChainableControllerInterface::state_interface_configuration)
      ...
      .def_readwrite("reference_interfaces_",
                     &PubChainableControllerInterface::reference_interfaces_);
}
